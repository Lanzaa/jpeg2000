...
impl ImageAndTileSizeMarkerSegment {
    ...
    fn num_x_tiles(&self) -> u32 {
        // TODO check if div_ceil is needed
...
#[derive(Debug, Clone)]
struct ProgressionRange {
    layer: Range<u16>,           // TODO value type
    component: Range<u16>,       // TODO value type
    resolution_level: Range<u8>, // TODO value type
    precinct: Range<u8>,         // TODO value type
}

#[derive(Debug)]
struct ProgressionState {
    order: ProgressionOrder,
    ranges: ProgressionRange,
    next_item: Option<ProgressionItem>,
}

impl Iterator for ProgressionState {
    type Item = ProgressionItem;

    fn next(&mut self) -> Option<Self::Item> {
        // grab the out item, if none should return early
        let out = self.next_item?;
        let ProgressionItem {
            mut layer,
            mut component,
            mut resolution_level,
            mut precinct,
        } = out.clone();
        match self.order {
            ProgressionOrder::LRLCPP => {
                // Layer, Res, Component, Precinct. check in opposite order
                precinct += 1;

                if precinct >= self.ranges.precinct.end {
                    precinct = self.ranges.precinct.start;
                    component += 1;
                }

                if component >= self.ranges.component.end {
                    component = self.ranges.component.start;
                    resolution_level += 1;
                }

                if resolution_level >= self.ranges.resolution_level.end {
                    resolution_level = self.ranges.resolution_level.start;
                    layer += 1;
                }

                if layer >= self.ranges.layer.end {
                    // end of progression
                    self.next_item = None;
                } else {
                    self.next_item = Some(ProgressionItem {
                        layer,
                        component,
                        resolution_level,
                        precinct,
            ...
            ProgressionOrder::RLLCPP => todo!("RLLCPP progression"),
            ProgressionOrder::RLPCLP => todo!("RLPCLP progresion"),
            ProgressionOrder::PCRLLP => todo!("ASDAS progression"),
            ProgressionOrder::CPRLLP => todo!("progression"),
            ProgressionOrder::Reserved { .. } => {
                todo!("Progression not implemented for reserved values")
            }
        }
        Some(out)
    }
...
impl ProgressionRange {
    fn first_item(&self) -> ProgressionItem {
        ProgressionItem {
            layer: self.layer.start,
            component: self.component.start,
            resolution_level: self.resolution_level.start,
            precinct: self.precinct.start,
        }
    }
}
...
fn into_iter(progression_order: ProgressionOrder, ranges: ProgressionRange) -> ProgressionState {
    ...
    let next_item = Some(ranges.first_item());
...
type HeaderIter = u8; // TODO handle passing around a reference to the packet headers in PPM or PPT
impl Tile {
    fn new(
        tile_bounds: Bounds,
        num_components: u16,
        ...
        let progression = cod.progression_order();

        if progression != ProgressionOrder::LRLCPP {
            todo!("need to handle other progresssion orders");
        }
        let num_layers = cod.no_layers();
        let style = &cod.coding_style_parameters;
        if !style.has_default_precinct_size() {
    ...
    fn consume<R: Read>(
        ...
        limit: u32,
        reader: &mut R,
        headers: Option<HeaderIter>,
    ) -> Result<(), DecodeError> {
        println!(
            "Need to read {} bytes from reader to decode packets in tile_part",
            limit
        );
        self.consume_count += 1;
        println!("Consume # {}", self.consume_count);
        // TODO -> debug
        info!(
            "Reading {} bytes for packets, headers passed in? {}",
            limit,
            headers.is_some()
        );
        let limited_reader = &mut reader.take(limit as u64);
        if headers.is_some() {
        ...
        for progression_ctx in x {
            //self.prointo_iter(ProgressionOrder::LRLCPP, ranges) {
            println!("progression_ctx : {:?}", progression_ctx);
            let ProgressionItem {
                layer: _, // Don't really need to know layer during decode
                component,
                resolution_level,
                precinct,
            } = progression_ctx;
            ...
            let Some(decoder) = self.precincts.remove(&key) else {
                todo!("panic?");
            };
            if let Some(_hi) = headers {
                todo!("headers iter");
            } else {
                let x = decoder
                    .consume_packet_header(limited_reader)?
                    .consume_packet(limited_reader)?;
                self.precincts.insert(key, x); //[&key] = x;
            }
        }
        Ok(())
    }
...
impl ContiguousCodestream {
    ...
    fn decode_tile_part<R: io::Read + io::Seek>(
        ...
        Ok(TilePart {
            header,
            data_offset,
            data_length: (data_end - data_offset) as u32,
        })
    }
...
impl<R: io::Read + io::Seek> Profile0Decoder<R> {
    pub fn new(mut reader: R) -> Result<Self, DecodeError> {
        let result = decode_jpc(&mut reader);
        let Ok(codestream) = result else {
            panic!(
                "TODO error message for codestream error {}",
                result.unwrap_err()
            );
        };
        ...
        let num_tiles = siz.num_y_tiles() * siz.num_x_tiles();
        let tiles = (0..num_tiles).map(|_| None).collect();
        let (width, height) = siz.image_dimensions();
        let num_components = siz.no_components();
        let mut components = Vec::with_capacity(num_components as usize);
        for ci in 0..num_components {
            let info = ComponentInfo {
                bit_depth: siz.precision(ci as usize).unwrap(),
                is_signed: siz.values_are_signed(ci as usize).unwrap(),
            };
    ...
    fn create_tile(&self, tile_index: u16, header: &TilePartHeader) -> Tile {
        let main_header = self.codestream.header();
        let siz = main_header.image_and_tile_size_marker_segment();
        let num_components = siz.no_components();
        let cod_main = self.codestream.header.coding_style_marker_segment();
        let tile_bounds = Bounds {
            x0: siz.tile_x0(tile_index as u32),
            y0: siz.tile_y0(tile_index as u32),
            x1: siz.tile_x1(tile_index as u32),
            y1: siz.tile_y1(tile_index as u32),
        };
        if !main_header.coding_style_component_segment().is_empty() {
            todo!("main COC, coding style component not implemented");
        }
        if let Some(first_headers) = header.first_headers.as_ref() {
            if first_headers.coding_style_marker_segment.is_some() {
                todo!("tile-part COD, coding style changes not implemented");
            }
            if !first_headers.coding_style_component_segment.is_empty() {
...
impl<R: io::Read + io::Seek> ImageDecoder for Profile0Decoder<R> {
    fn info(&self) -> &ImageInfo {
        &self.image_info
    }

    fn decode_component(&mut self, component: u16, buf: &mut [u8]) -> Result<(), DecodeError> {
        // Profile 0 restrictions, see Table A.45
        // single tile over full image
        // dimensions <= 128x128
        // Single precinct
        // codeblocks: xcb = ycb = 5 or xcb = ycb = 6, ie 32x32 or 64x64

        assert_eq!(
            1, self.image_info.num_components,
            "Needs more work for multi-component"
        );

        let siz = self.codestream.header.image_and_tile_size_marker_segment();
        assert_eq!(1, siz.num_x_tiles(), "Expecting a single tile");
        assert_eq!(1, siz.num_y_tiles(), "Expecting a single tile");
        assert_eq!(1, siz.no_components(), "Expecting a single component");

        assert_eq!(
            1,
            siz.horizontal_separation(0).expect("oof"),
            "not implemented"
        );
        assert_eq!(
            1,
            siz.vertical_separation(0).expect("oof"),
            "not implemented"
        );

        println!("Single tile dimensions: {:?}", siz.tile_dimensions(0));

        let tile_parts = &self.codestream.tile_parts;
        ...
        for (idx, tile_part) in tile_parts.iter().enumerate() {
            ...
            let tile_part_index = sot.tile_part_index[0];
            assert_eq!(
                tile_part_index as usize, idx,
                "Expected tile-parts to be in order"
            );
            if tile_part_index == 0 {
                let tile = self.create_tile(tile_index, header);
                self.tiles[tile_index as usize] = Some(tile);
            }
            let Some(tile) = self.tiles[tile_index as usize].as_mut() else {
                panic!("exepcctedfaksdfj ashdfasdfa ");
            };

            let no_tp = sot.no_tile_parts[0];

            if header.progression_order_change.is_some() {
                todo!("tile-part POC, progression order changes not implemented");
            }
        ...
        // TODO handle IDWT
        // TODO handle levle shift?
        let zzz = &self.tiles[0].as_ref().expect("ASD").precincts;
        println!("Wow: {:?}", zzz);
...
mod tests {
    ...
    #[test]
    fn test_progression_iter() {
        // TODO maybe we should test this
        let ranges = ProgressionRange {
            layer: (0..2),
            component: (0..1),
            resolution_level: (0..3),
            precinct: (0..1),
        };
        let next_item = Some(ranges.first_item());
        let prog = ProgressionState {
            order: ProgressionOrder::LRLCPP,
            ranges: ranges.clone(),
            next_item,
        };
        let items: Vec<ProgressionItem> = prog.into_iter().collect();
        // a better test might be a good idea
